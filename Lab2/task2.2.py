...
for sts_start in sts_starts:
    ...
    # 1. 提取每个符号上导频的接收值（假设信道均衡后）
    pilot_received = equalized_symbols[pilot_indices, :]  # shape: (4, num_symbols)

    # 2. 已知导频理想值（例如 [1, 1, 1, -1]）
    # 要确保 pilot_known 是 shape = (4,), dtype = complex
    # 如果你已经有了，就跳过这步
    pilot_known = np.array([1, 1, 1, -1], dtype=complex)

    # 3. 计算每个符号上的导频相位误差（angle of received / known）
    pilot_phase_errors = np.angle(pilot_received / pilot_known[:, np.newaxis])  # shape: (4, num_symbols)

    # 4. 取每个符号的平均相位偏差作为补偿角度（不回归、不插值）
    # shape: (num_symbols,)
    avg_phase_per_symbol = np.mean(pilot_phase_errors, axis=0)

    # 5. 对每个 OFDM 符号做统一相位补偿
    phase_correction = np.exp(-1j * avg_phase_per_symbol)  # shape: (num_symbols,)
    equalized_symbols_cfo_corrected = equalized_symbols * phase_correction[np.newaxis, :]

    # 7. 提取所有数据子载波，flatten
    data_symbols = equalized_symbols_cfo_corrected[data_subcarriers, :]
